    ...
    >
      <div className="mx-auto w-full max-w-300">
        ...
        {isLoading ? (
          <div
            data-portfolio-grid
            className="grid grid-cols-1 gap-3.5 md:grid-cols-2 lg:gap-6"
          >
          ...
          {projects.slice(0, showcaseLimit).map((project, idx) => (
            ...
            >
              ...
              >
                ...
                <div
                  className={`absolute inset-0 ${toneClass[idx % toneClass.length]} opacity-100 transition-opacity duration-300 group-hover:opacity-0`}
                  aria-hidden="true"
                />
              </div>
              <h3 className="mb-2 mt-3 font-['Syne'] text-[34px]">
                {project.title}
              </h3>
              <p className="m-0 max-w-[28ch] text-[16px] text-[#2f2f2f]">
                {project.summary ||
                  project.description ||
                  "Click to view project details and README summary"}
              </p>
