...
const LINES = [
  ...
  },
  {
    ...
    prompt: false,
    cls: "text-[#666]",
    text: "remote: Enumerating objects: 42, done.",
  },
  {
    delay: 1300,
    prompt: false,
    cls: "text-[#666]",
    text: "remote: Counting objects: 100% (42/42), done.",
  },
  {
    delay: 1700,
    prompt: false,
    cls: "text-[#6ec6f5]",
    text: "Receiving objects: 100% (42/42)",
  },
  {
    delay: 2200,
    prompt: false,
    cls: "text-[#f5c842]",
    text: "Resolving deltas: 100% (18/18), done.",
  },
  ...
  },
  {
    ...
    prompt: false,
    cls: "text-[#e07fd4]",
    text: "merge conflict: too much talent",
  },
  {
    delay: 4200,
    prompt: false,
    cls: "text-[#f97b50]",
    text: "force pushing greatness...",
  },
  {
    delay: 4800,
    prompt: false,
    cls: "text-[#c8f060]",
    text: "✓ portfolio loaded. you're welcome.",
  },
...
const STARS = [
  ...
  {
    style: { bottom: "55px", left: "20px" },
    size: 26,
    color: "#f97b50",
    delay: "0.7s",
  },
  {
    style: { bottom: "52px", right: "35px" },
    size: 28,
    color: "#6ec6f5",
    delay: "1.1s",
  },
  ...
  return (
    <div className="relative min-h-screen bg-[#e8e8d8] flex flex-col items-center justify-center overflow-hidden font-['Space_Grotesk',sans-serif] rounded-xl border-2 border-[#1a1a1a]">
      {STARS.map((s, i) => (
        <span
          key={i}
          className="absolute [animation:var(--animate-star-pop)]"
          style={{ ...s.style, animationDelay: s.delay } as CSSProperties}
        >
          <svg width={s.size} height={s.size} viewBox="0 0 22 22">
            <path
              d="M11 1l2.4 7.4H21l-6.2 4.5 2.4 7.4L11 16l-6.2 4.3 2.4-7.4L1 8.4h7.6z"
              fill={s.color}
              stroke="#1a1a1a"
              strokeWidth="1.2"
            />
      ...
      <div className="w-[min(500px,90%)] bg-[#1a1a1a] rounded-xl border-2 border-[#333] overflow-hidden [animation:var(--animate-fade-up)]">
        ...
        <div className="bg-[#2a2a2a] px-3.5 py-2.5 flex items-center gap-1.5 border-b border-[#333]">
          ...
          <span className="ml-2 text-[12px] text-[#666] font-mono">
            asad@portfolio ~ main
          </span>
        </div>
        ...
        <div
          ref={termRef}
          className="p-5 font-mono text-[13px] leading-[1.8] min-h-[180px] overflow-y-auto"
        >
          {visibleLines.map((line, i) => (
            <div
              key={i}
              className="flex items-start gap-2 opacity-0 [animation:var(--animate-line-in)]"
            >
              {line.prompt && (
                <span className="text-[#c8f060] shrink-0">❯</span>
              )}
              {line.cursor ? (
                <span>
        ...
        <div className="h-1.5 bg-[#ccc] rounded-full overflow-hidden border-[1.5px] border-[#1a1a1a]">
          <div
            ...
            style={{
              width: `${prog.pct}%`,
              background:
                "linear-gradient(90deg,#c8f060,#f97b50,#f5c842,#6ec6f5,#e07fd4)",
            }}
          />
        </div>
      </div>
